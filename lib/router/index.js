...
export default class Router {
  ...
  findRelated(req, res) {
    ...
    Model.findById(id)
      ...
      .exec((err, resource) => {
        ...
        const relationship = getRelationship(this.fields, type, field);
        const relationshipType = relationship.type;
        const relationshipIsMany = relationship.isMany;
        const populatedString = populateString(this, relationshipType);

        resource.populate(populatedString, (populateErr, populatedResource) => {
          if (populateErr) {
            return res.sendStatus(500);
          }

          if (relationshipIsMany) {
            // FIXME: need to add count and pagination
            return res.json(serializer.serializeResponse(this, 'get', relationshipType, populatedResource[field].toObject()));
          }

          // FIXME: doesn't serialize the included array properly
          return res.json(serializer.serializeResponse(this, 'get', relationshipType, populatedResource[field].toObject()));
        });
      });
  }
  ...
  setupRoutes() {
    ...
    const fieldEndpoint = `/:type/:id/:field`;
    ...
    router.route(fieldEndpoint)
      .get((req, res) => {
        this.findRelated(req, res);
      });
  }
}
