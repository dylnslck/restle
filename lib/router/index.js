...
export default class Router {
  ...
  createResource(req, res) {
    ...
    Model.create(serialized, (createErr, createdResource) => {
      if (createErr) {
        if (createErr.name === 'ValidationError' || createErr.name === 'MongoError') {
          return res.status(400).json({
            errors: [{
              status: 400,
              title: 'Validation error',
              detail: 'Creating this resource failed because either a required field was not present or a unique entry was attempted to be duplicated.',
            }],
          });
        }
