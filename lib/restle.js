...
Restle.prototype = {
  ...
  _init: function(options) {
    ...
    // Initialize database.
    // TODO: modularize database url, i.e. username,
    // password, port, etc.
    if(this.options.databaseUrl) {
      mongoose.connect(this.options.databaseUrl, {
        server: {
          auto_reconnect: true,
          socketOptions: {
            keepAlive: 1
      ...
      mongoose.connection.on('connected', function() {
        console.log('Database connected to ' + options.databaseUrl);
      });
    } else {
      console.log('Warning: no database has been connected')
    }
    
    this.app.use(bodyParser.json());
    this.app.use('/api', this.router);
    this.app.listen(this.options.port || this._defaults.port);

    if(this.options.verbose) {
      console.log('Restle listening on port ' + this.options.port || this._defaults.port);
    }
  },
